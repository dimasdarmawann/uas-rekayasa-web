...
<main class="content">
    <div class="stats">
        ...
        <div class="stat-card">
            <div class="num">{{ $hariList }}</div>
            <div class="lbl">Hari Aktif</div>
        </div>
        <div class="stat-card">
            <div class="num">{{ $kegiatans->pluck('pembina')->unique()->count() }}</div>
            <div class="lbl">Pembina</div>
        </div>
    </div>

    <div class="section-title">{{ $total }} kegiatan tersedia</div>

    @if($kegiatans->count() > 0)
    <div class="card-grid" id="cardGrid">
        @foreach($kegiatans as $kegiatan)
        @php $hariClass = 'hari-' . strtolower(str_replace(' ', '', $kegiatan->hari)); @endphp
        <div class="ekskul-card {{ $hariClass }}"
             data-nama="{{ strtolower($kegiatan->nama_kegiatan) }}"
             data-hari="{{ strtolower($kegiatan->hari) }}"
             onclick="openModal(
            ...
            <div class="card-body">
                <div class="card-title">{{ $kegiatan->nama_kegiatan }}</div>
                <div class="card-info">
                    <div class="info-row">
                        <i class="ti ti-calendar-week"></i>
                        <span class="badge-hari">{{ ucfirst($kegiatan->hari) }}</span>
                    </div>
                    <div class="info-row">
...
<footer>
    <strong>EkskulManager</strong> &mdash; Sistem Manajemen Kegiatan Ekstrakurikuler &copy; {{ date('Y') }}
</footer>
