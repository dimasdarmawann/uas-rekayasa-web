...
<div class="content">
    <table style="margin-bottom:18px;">
        <tr>
            <td style="width:33%;border:1.5px solid #e2e8f0;padding:11px 14px;text-align:center;border-radius:7px;">
                <div style="font-size:20px;font-weight:800;color:#4f46e5;">{{ $kegiatans->count() }}</div>
                <div style="font-size:10px;color:#64748b;">Total Kegiatan</div>
            </td>
            <td style="width:4%;"></td>
            <td style="width:33%;border:1.5px solid #e2e8f0;padding:11px 14px;text-align:center;border-radius:7px;">
                <div style="font-size:20px;font-weight:800;color:#10b981;">{{ $kegiatans->pluck('hari')->unique()->count() }}</div>
                <div style="font-size:10px;color:#64748b;">Hari Aktif</div>
            </td>
            <td style="width:4%;"></td>
            <td style="width:33%;border:1.5px solid #e2e8f0;padding:11px 14px;text-align:center;border-radius:7px;">
                <div style="font-size:20px;font-weight:800;color:#f59e0b;">{{ $kegiatans->pluck('pembina')->unique()->count() }}</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td style="font-weight:700;color:#6366f1;text-align:center;">{{ $i+1 }}</td>
                <td style="font-weight:600;">{{ $k->nama_kegiatan }}</td>
                <td><span class="badge">{{ $k->hari }}</span></td>
                <td>{{ \Carbon\Carbon::parse($k->waktu)->format('H:i') }} WIB</td>
                <td>{{ $k->pembina }}</td>
            </tr>
            @empty
    ...
    <div class="footer">
        <div>
            <div>Dicetak oleh: <strong>{{ session('username','Admin') }}</strong></div>
            <div>Ekskul Manager &copy; {{ date('Y') }}</div>
        </div>
        <div class="sign-box">
            <div>Mengetahui,</div>
